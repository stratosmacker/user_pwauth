...
class OC_USER_PWAUTH extends OC_User_Backend implements OC_User_Interface {
	...
	public function __construct() {
		...
		//	}
		//}
        //New stuff to accomodate CS logins

        $this->pwauth_bin_path = OC_Appconfig::getValue('user_pwauth', 'pwauth_path', OC_USER_BACKEND_PWAUTH_PATH);
        $r=array();
        //Use yp cat to get list of user ids
        $shellout=array();
        $shellreturn=0;
        exec ( "ypcat passwd | sort -t: -k 3,3 -n" ,$shellout, $shellreturn);
        if($shellreturn == 0){//success
            foreach($shellout as $entry){
                $tok = explode(":", $entry);
                $rval = intval($tok[2]);
                if($rval){//we have a UID
                    $r[] = $rval;
                }
            }
        }
